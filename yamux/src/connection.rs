            ...
            let mut window_update = None;
            {
                ...
                if matches!(self.config.window_update_mode, WindowUpdateMode::OnReceive) {
                    if let Some(credit) = shared.next_window_update() {
                        ...
                        // TODO: Handle credit being > u32::MAX. Even better remove OnReceive!
                        let mut frame = Frame::window_update(stream_id, credit as u32);
        ...
        if let Some(s) = self.streams.get_mut(&stream_id) {
            ...
            if matches!(self.config.window_update_mode, WindowUpdateMode::OnReceive) {
                if let Some(credit) = shared.next_window_update() {
                    shared.window += credit;
                    // TODO: Handle credit being > u32::MAX. Even better remove OnReceive!
                    let frame = Frame::window_update(stream_id, credit as u32);
